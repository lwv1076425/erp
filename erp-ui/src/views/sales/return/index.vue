<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb8"><el-col :span="1.5"><el-button type="primary" plain icon="Plus" @click="handleAdd">新增退货</el-button></el-col></el-row>
    ...
    <el-dialog v-model="dialog.visible" title="新增销售退货" width="500px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="客户ID" prop="customerId"><el-input-number v-model="form.customerId" :min="1" /></el-form-item>
...
import { listSalReturn, addSalReturn, deleteSalReturn } from '@/api/sales/return'
...
const form = reactive({ customerId: null, returnDate: '', totalAmount: 0, remark: '' })
const rules = { customerId: [{ required: true, message: '请输入客户ID', trigger: 'blur' }] }
onMounted(() => { getList() })
const getList = async () => { loading.value = true; try { const res = await listSalReturn(queryParams); list.value = res.data.records; total.value = res.data.total } finally { loading.value = false } }
const handleAdd = () => { form.customerId = null; form.returnDate = ''; form.totalAmount = 0; form.remark = ''; dialog.visible = true }
const handleDelete = async (row) => { try { await ElMessageBox.confirm('确认删除？', '提示', { type: 'warning' }); await deleteSalReturn(row.id); ElMessage.success('删除成功'); getList() } catch {} }
const submitForm = async () => { try { await formRef.value.validate(); await addSalReturn(form); ElMessage.success('新增成功'); dialog.visible = false; getList() } catch {} }
