<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb8"><el-col :span="1.5"><el-button type="primary" plain icon="Plus" @click="handleAdd">新增盘点</el-button></el-col></el-row>
    ...
    <el-dialog v-model="dialog.visible" title="新增盘点" width="500px" append-to-body>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="仓库ID" prop="warehouseId"><el-input-number v-model="form.warehouseId" :min="1" /></el-form-item>
        <el-form-item label="盘点日期"><el-date-picker v-model="form.checkDate" type="date" value-format="YYYY-MM-DD" /></el-form-item>
...
import { listInvCheck, addInvCheck, deleteInvCheck } from '@/api/inventory/check'
...
const form = reactive({ warehouseId: null, checkDate: '', remark: '' })
const rules = { warehouseId: [{ required: true, message: '请输入仓库ID', trigger: 'blur' }] }
onMounted(() => { getList() })
const getList = async () => { loading.value = true; try { const res = await listInvCheck(queryParams); list.value = res.data.records; total.value = res.data.total } finally { loading.value = false } }
const handleAdd = () => { form.warehouseId = null; form.checkDate = ''; form.remark = ''; dialog.visible = true }
const handleDelete = async (row) => { try { await ElMessageBox.confirm('确认删除？', '提示', { type: 'warning' }); await deleteInvCheck(row.id); ElMessage.success('删除成功'); getList() } catch {} }
const submitForm = async () => { try { await formRef.value.validate(); await addInvCheck(form); ElMessage.success('新增成功'); dialog.visible = false; getList() } catch {} }
